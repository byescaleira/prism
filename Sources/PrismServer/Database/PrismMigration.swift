...
public struct PrismMigrator: Sendable {
        ...
        for migration in migrations where !applied.contains(migration.version) {
            try await db.transaction { db in
                ...
                try db.execute(
                    ...
                    parameters: [.int(migration.version), .text(migration.name), .text(ISO8601DateFormatter().string(from: .now))]
                )
            }
        }
    }
    ...
    public func rollback() async throws {
        ...
        guard let lastVersion = applied.max(),
              let migration = migrations.first(where: { $0.version == lastVersion }) else {
            return
        }
