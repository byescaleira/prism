...
<Tip>
  For API-first applications, use `.html()` primarily for admin dashboards, error pages, and occasional server-rendered pages. Pair it with [Content Negotiation](/content/content-negotiation) to serve HTML to browsers and JSON to API clients from the same endpoint.
</Tip>
